...
type Server struct {
	...
	OAuthStatus       string            `json:"oauth_status,omitempty"`         // OAuth status: "authenticated", "expired", "error", "none", "unknown"
	...
	// EffectiveStatus / LastSuccessAt / LastAuthFailureAt are the
	// freshness-aware surfaces added by the status-staleness fix (design doc
	// R1/R3, implementation plan A2/A3). Status stays exactly as it was for
	// wire compatibility; these are additive. See health.DeriveEffectiveStatus.
...
type OAuthConfig struct {
	...
	// TokenValid deliberately has NO omitempty (D3.1 / A4): a bool's zero
	// value is `false`, which omitempty drops from the wire, making
	// "token_valid: false" indistinguishable from "no token field at all."
	// false must reach the client.
	TokenValid bool `json:"token_valid"` // Whether token is currently valid
}
